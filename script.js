...
function mostrarSeccionApp(seccion) {
  ...
  if (gastronomia) gastronomia.style.display = 'none';
 ...
 switch (seccion) {
  case 'inicio':
    ...
    if (gastronomia) gastronomia.style.display = 'block';
    break;
  case 'clima':
...
const platosData = [
  {region:"Caribe",nombre:"Arepas de Huevo",descripcion:"Arepa frita rellena de huevo, típica de la costa atlántica",tipo:"Desayuno",precio:"$8,000",icon:"🥚", imagen:"hero40.jpg"},
  {region:"Caribe",nombre:"Sancocho de Pescado",descripcion:"Sopa tradicional con pescado fresco y plátano",tipo:"Almuerzo",precio:"$25,000",icon:"🐟", imagen:"sancochodepescado.jpg"},
  {region:"Andina",nombre:"Bandeja Paisa",descripcion:"Plato completo con frijoles, carne, chicharrón y más",tipo:"Almuerzo",precio:"$30,000",icon:"🍛", imagen:"bandeja.jpg"},
  {region:"Andina",nombre:"Ajiaco Santafereño",descripcion:"Sopa cremosa de papa con pollo y mazorca",tipo:"Almuerzo",precio:"$20,000",icon:"🥘", imagen:"ajiaco.jpg"},
  {region:"Pacífica",nombre:"Encocado de Camarón",descripcion:"Camarones en salsa de coco con arroz",tipo:"Almuerzo",precio:"$35,000",icon:"🦐", imagen:"camaron.jpg"},
  {region:"Pacífica",nombre:"Empanadas de Pipián",descripcion:"Empanadas rellenas de papa y maní",tipo:"Snack",precio:"$3,000",icon:"🥟", imagen:"empanada.jpg"},
  {region:"Amazónica",nombre:"Pirarucú Asado",descripcion:"Pescado amazónico asado con yuca",tipo:"Almuerzo",precio:"$40,000",icon:"🐠", imagen:"Asado.jpg"},
  {region:"Amazónica",nombre:"Casabe con Hormiga",descripcion:"Pan de yuca con hormigas culonas",tipo:"Snack",precio:"$15,000",icon:"🍪", imagen:"casabe.jpg"},
  {region:"Andina",nombre:"Tamales",descripcion:"masa de maíz con carne y verduras envuelta en hoja de plátano.",tipo:"Desayuno",precio:"$18,000",icon:"🐷", imagen:"tamales.jpg"},
  {region:"Caribe",nombre:"Arroz con coco y pescado frito",descripcion:"arroz dulce con coco, pescado y patacones.",tipo:"Desayuno",precio:"$12,000",icon:"🐠", imagen:"coco.jpg"},
];

let platosActuales = platosData;
let regionActual = 'all';

function renderizarPlato(plato) {
  return `
    <div class="gastronomia-card" data-region="${plato.region}">
      <div class="gastronomia-card__image">
        ${plato.imagen ? `<img src="${plato.imagen}" alt="${plato.nombre}">` : '<i class="fa-solid fa-utensils"></i>'}
      </div>
      <div class="gastronomia-card__content">
        <span class="gastronomia-card__region">${plato.region}</span>
        <h3 class="gastronomia-card__name">${plato.nombre}</h3>
        <p class="gastronomia-card__description">${plato.descripcion}</p>
        <div class="gastronomia-card__info">
          <span class="gastronomia-card__tipo"><i class="fa-solid fa-clock"></i>${plato.tipo}</span>
          <span class="gastronomia-card__precio">${plato.precio}</span>
        </div>
      </div>
    </div>
  `;
...
function mostrarPlatos(region = 'all') {
  const gastroGrid = document.getElementById('gastronomia-grid');
  if (!gastroGrid) return;
  
  let platosFiltrados = region === 'all' ? platosData : platosData.filter(p => p.region === region);
  gastroGrid.innerHTML = platosFiltrados.map(plato => renderizarPlato(plato)).join('');
}
...
const profileToggles = document.querySelectorAll('.profile__toggle');
